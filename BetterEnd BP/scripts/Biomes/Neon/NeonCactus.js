...
world.beforeEvents.worldInitialize.subscribe((data) => {
    data.blockComponentRegistry.registerCustomComponent("betterend:neon_cactus", {
        onTick({ block }) {
            const sizeState = (block) => block.permutation?.getState("betterend:sizes");
            const updateState = (block, size, rot) => {
                block.setPermutation(BlockPermutation.resolve(block.typeId, {
                    "betterend:sizes": size,
                    "minecraft:facing_direction": rot,
                }));
            };
            const rot = block.permutation?.getState("minecraft:facing_direction");
            ...
            for (const side of sides) {
                if (side.typeId !== "betterend:neon_cactus")
                    continue;
            ...
            for (const top of tops) {
                if (top.typeId !== "betterend:neon_cactus")
                    continue;
